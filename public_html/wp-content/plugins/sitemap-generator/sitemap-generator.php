...
/*
Plugin Name: Dagon Design Sitemap Generator (ed)
Author: Dagon Design
Version: 3.17
*/
 ...
 */
add_option('ddsg_language', 'Russian');
add_option('ddsg_items_per_page', 0);
...
include __DIR__ . '/lang/' . $ddsg_lang_file . '.php';
...
function ddsg_options_page() {
	...
	<p>
		<?php echo DDSG_FOR_INFO; ?><br />
		<span>http://www.dagondesign.com/articles/sitemap-generator-plugin-for-wordpress/</span>
	</p>
...
function ddsg_create_sitemap() {
	...
	if( is_array($tags) && count($tags) ) {
		$t_out .= '<h2>Метки</h2>'.PHP_EOL;
		$t_out .= '<ul>'.PHP_EOL;
		foreach( $tags as $tagItem ) {
			$tagLink = get_tag_link( $tagItem );
			$t_out .= '<li>';
			$t_out .= '<a href="'.$tagLink.'">'.($tagItem->name).'</a>';
			$t_out .= '</li>';
		}
		$t_out .= '</ul>'.PHP_EOL;
	}
