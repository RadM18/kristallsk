...
if(!function_exists( 'codepeople_add_promote_banner' ))
{
	function codepeople_add_promote_banner($wp_admin_bar)
	{
		...
		if( empty($codepeople_promote_banner_plugins) ) return;
		...
		if(isset($_POST['codepeople_promote_banner_nonce']) && wp_verify_nonce($_POST['codepeople_promote_banner_nonce'], __FILE__))
		{
			...
			)
			{
				...
				{
					print '<script>jQuery(function(){jQuery("<form action=\"'.esc_attr($codepeople_promote_banner_plugins[$_POST['codepeople_promote_banner_plugin']]['plugin_url']).'\" target=\"_blank\" style=\"display:none !important\"></form>").appendTo("body").submit();});</script>';
				}
			}
		}

		$minimum_days = 86400000*7;
		$now = time();
				...
				<div id="codepeople-review-banner">
					...
					<div class="codepeople-review-banner-content">
						...
						<div class="codepeople-review-banner-buttons">
							<form method="post">
								<button class="main-button">Publish a Review</button>
